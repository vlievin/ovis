...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', default=1000, type=int, help='number of epochs (use n_steps if `epochs` < 0)')
    ...
    # estimator
    parser.add_argument('--estimator', default='reinforce', help='[vi, reinforce, vimco, gs, st-gs, tvo]')
    ...
    parser.add_argument('--partition', default=21, type=int, help='choice of partition of unit interval for TVO estimator')
    ...
    parser.add_argument('--iw_test', default=10, type=int, help='number of Importance-Weighted samples for testing')
    ...
    # latent space
    parser.add_argument('--num_latents', default=1, help='Number of latent variables')
    ...
    # model architecture
    parser.add_argument('--target_val', default=0.499, help='Target value')
    ...
    run_id = f"{opt.optimizer}-{opt.estimator}-seed{opt.seed}-iw{opt.iw}-mc{opt.mc}"
    # f"{opt.dataset}-{opt.optimizer}-{opt.model}-{opt.prior}-{opt.estimator}-seed{opt.seed}"
    # if opt.mini:
    #     run_id = "mini-" + run_id
    if len(opt.id) > 0:
        run_id += f"-{opt.id}"
    ...
    if opt.estimator == 'tvo':
        run_id += f"-tvopart{opt.partition}"
    # if opt.learn_prior:
    #     run_id += "-learn-prior"
    ...
    try:
        ...
        target = torch.Tensor(1, opt.num_latents)
        target.fill_(opt.target_val)
        dset_train = [target]
        dset_valid = [target]
        dset_test = [target]
        print(dset_train)
        ...
        x = dset_train[0]
        base_logger.info(f"Sample: x.shape = {x.shape}, x.min = {x.min():.3f}, x.max = {x.max():.3f}, x.dtype = {x.dtype}")

        # dataloaders
        loader_train = DataLoader(dset_train, batch_size=opt.bs, shuffle=True, num_workers=opt.workers, pin_memory=True)
        loader_valid = DataLoader(dset_train, batch_size=opt.bs, shuffle=True, num_workers=opt.workers, pin_memory=True)
        ...
        estimator = Estimator(baseline=baseline, mc=opt.mc, iw=opt.iw, partition=opt.partition)
        ...
        base_logger.info(f"running for {epochs} epochs, {iter_per_epoch * epochs} steps")
        ...
        for epoch in range(1, epochs + 1):
            ...
            thetas = torch.sigmoid(model.qlogits.detach()).cpu().numpy()
            plot_loss = thetas * (1 - opt.target_val) ** 2 # Loss = E_p [ f(x) ], f(x) = (x-t)**2
            plot_loss += (1 - thetas) * opt.target_val ** 2 # Loss = E_p [ f(x) ], f(x) = (x-t)**2
