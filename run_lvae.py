...
parser.add_argument('--freebits', default=1., type=float, help='freebits')

# latent space
parser.add_argument('--L', default=3, type=int, help='number of layers of stochastic units')
parser.add_argument('--N', default=4, type=int, help='number of latent variables')
...
run_id = f"shapes-LVAE-{opt.estimator}-fb{opt.freebits}-seed{opt.seed}"
if len(opt.id) > 0:
    run_id += f"-{opt.id}"
...
# define model
torch.manual_seed(opt.seed)
latents = opt.L * [(opt.N, opt.K,)]
model = LVAE(x.shape, latents, opt.hdim, kdim=opt.kdim, nlayers=opt.nlayers, learn_prior=opt.learn_prior, use_baseline=opt.baseline)
...
estimator = Estimator(baseline=baseline, mc=opt.mc, iw=opt.iw, N=opt.N, K=opt.K, hdim=opt.hdim, freebits=opt.freebits)
...
# optimizer
optimizers = []
optimizers += [torch.optim.Adam(list(model.parameters()) + list(estimator.parameters()), lr=opt.lr)]
# todo (temprorary for layer wise baseline) if len(list(estimator.parameters())):
